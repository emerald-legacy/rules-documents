...
=== Conflict Action, Triggered Conflict Ability
...
- Any additional deckbuilding restrictions contained in the separate link:{edict-url}[Emerald Edict] document, based on the format being played, must be followed.
...
=== Ephemeral
_Ephemeral_ is a keyword ability found on event and attachment cards.
On events, it means that after the card has been played, it is removed from the game.
On attachments, it means that when the attachment leaves play, it is removed from the game.
In either case, effects that would normally put the card in another out-of-play area cannot prevent the card being removed from the game.
...
=== Injure
When a game effect instructs a player to injure a character it means:

"If the character has no fate on it, discard it. Otherwise, remove 1 fate from it."
...
=== Peaceful
_Peaceful_ is a keyword ability found on event and attachment cards.
It means that this card cannot be played during a conflict.
...
=== Thriving
_Thriving_ is a keyword ability found on Dynasty cards.
It means when the fate phase ends, if this is the only card in this province, fill this province with 1 facedown dynasty card.
...
=== I. Dynasty Phase
[.timing-chart]
[cols="1", frame=none, grid=none, stripes=none]
|===
|{set:cellbgcolor:#e7e3d9}
...
|{set:cellbgcolor:#f7e2d0}
[.tc-window]#1.4 SPECIAL ACTION WINDOW# +
...
|{set:cellbgcolor:#e2dbf0}
[.tc-loop]#*3.3* Conflict Ends / Conflict was passed. Return to
...
|{set:cellbgcolor:#f7e2d0}
[.tc-window]#3.2.2 CONFLICT ACTION WINDOW# +
...
|{set:cellbgcolor:#e2dbf0}
[.tc-loop]#*3.2.8* Return home. Go to (*3.3*).#
|===
